...
Solve::Solve(const std::string& particleFile, const std::string& nodeFile)
{
    ...
    while (std::getline(file, line))
    {
        ...
        if (keyword == "Material") {
            ...
            std::istringstream iss_props(line);
            double rho, K, G;
            iss_props >> rho >> K >> G;
            if (iss_props.fail() || rho <= 0 || K <= 0 || G <= 0) {
                std::cerr << "Error: Invalid material properties for material ID " << materialID << std::endl;
                continue;
            }
            //std::cout << "Reading Material ID " << materialID
            //    << ": rho = " << rho << ", K = " << K << ", G = " << G << std::endl;
        ...
        else if (keyword == "Particle")
        {
            ...
            mesh = std::make_unique<Mesh>(nodeFile, materials[currentParticleID]); // Pass shared_ptr
            // std::cout << "Mesh created for particle: " << particleID << std::endl;
        }
        else if (mesh)
        {
            mesh->initParticleInfo(line);
...
void Solve::updateParticles(std::unique_ptr<Mesh>& msh)
{
    ...
    {
        Particle& ip = msh->particles[it.first];
        Material& material = *(msh->material);
